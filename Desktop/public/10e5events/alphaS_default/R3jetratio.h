...
// Tue Apr 20 17:14:33 2021 by ROOT version 6.22/08
// from TTree events/events
// found on file: ISRoff__R3.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   R3jetratio(TTree *tree=0);
   virtual ~R3jetratio();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop_antikt();
   virtual void     Loop_kt();
   virtual void     Loop_ee_genkt();
   virtual void     Loop_valencia();
   virtual void     Loop_cambridge();
...
#ifdef R3jetratio_cxx
R3jetratio::R3jetratio(TTree *tree) : fChain(0)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ISRoff__R3.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("ISRoff__R3.root");
      }
...
R3jetratio::~R3jetratio()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t R3jetratio::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t R3jetratio::LoadTree(Long64_t entry)
{
...
void R3jetratio::Init(TTree *tree)
{
...
void R3jetratio::Show(Long64_t entry)
{
...
Int_t R3jetratio::Cut(Long64_t entry)
{
